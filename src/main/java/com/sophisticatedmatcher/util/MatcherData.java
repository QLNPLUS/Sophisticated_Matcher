...
public final class MatcherData {
    ...
    public static Rule decodeRule(CompoundTag encoded) {
        ...
        if (encoded.contains(PATH_KEY)) {
            ListTag rawPath = encoded.getListOrEmpty(PATH_KEY);
            for (int index = 0; index < rawPath.size(); index++) {
                CompoundTag part = rawPath.getCompoundOrEmpty(index);
                path.add("index".equals(part.getStringOr("kind", ""))
                        ? PathSegment.index(part.getIntOr("value", -1))
                        : PathSegment.key(part.getStringOr("value", "")));
            }
        }
        return new Rule(path, Operator.fromId(encoded.getStringOr(OPERATOR_KEY, "equals")),
                encoded.get(EXPECTED_KEY), encoded.get(MIN_KEY), encoded.get(MAX_KEY),
                !encoded.contains(MIN_INCLUSIVE_KEY) || encoded.getBooleanOr(MIN_INCLUSIVE_KEY, true),
                !encoded.contains(MAX_INCLUSIVE_KEY) || encoded.getBooleanOr(MAX_INCLUSIVE_KEY, true));
    }

    public static Rule selectedRule(ItemStack matcher) {
        CompoundTag data = getData(matcher);
        if (data.contains(RULE_KEY)) return decodeRule(data.getCompoundOrEmpty(RULE_KEY));
        String key = data.getStringOr(NBT_KEY, "");
        Tag value = data.get(VALUE_KEY);
        if (key.isEmpty()) key = data.getStringOr(COMPONENT_KEY, "");
    ...
    private static Tag resolve(DataComponentMap components, List<PathSegment> path) {
        ...
        Identifier id;
        try {
            id = Identifier.parse(path.get(0).key());
    ...
    public static String valueText(Tag value) {
        if (value instanceof ByteTag byteTag) return Byte.toString(byteTag.byteValue()) + "b";
        if (value instanceof ShortTag shortTag) return Short.toString(shortTag.shortValue()) + "s";
        if (value instanceof IntTag intTag) return Integer.toString(intTag.intValue());
        if (value instanceof LongTag longTag) return Long.toString(longTag.longValue()) + "l";
        if (value instanceof FloatTag floatTag) return Float.toString(floatTag.floatValue()) + "f";
        if (value instanceof DoubleTag doubleTag) return Double.toString(doubleTag.doubleValue()) + "d";
    ...
    private static BigDecimal numericValue(NumericTag value) {
        if (value instanceof ByteTag byteTag) return BigDecimal.valueOf(byteTag.byteValue());
        if (value instanceof ShortTag shortTag) return BigDecimal.valueOf(shortTag.shortValue());
        if (value instanceof IntTag intTag) return BigDecimal.valueOf(intTag.intValue());
        if (value instanceof LongTag longTag) return BigDecimal.valueOf(longTag.longValue());
        if (value instanceof FloatTag floatTag) return BigDecimal.valueOf(floatTag.floatValue());
        return BigDecimal.valueOf(((DoubleTag) value).doubleValue());
    }

    private static DataComponentType<?> componentType(List<PathSegment> path) {
        if (path.isEmpty() || path.get(0).listIndex()) return null;
        try {
            return BuiltInRegistries.DATA_COMPONENT_TYPE.getOptional(Identifier.parse(path.get(0).key()))
                    .orElse(null);
        } catch (RuntimeException exception) {
            return null;
        }
